...
/*
 * This is essentially the same as pure C sample, but
 * with callbacks to golang code.
 */
...
/*
 * Any golang functions we call must be declared.
 * Some translation between go and C syntax will
 * be necessary as expected. To be able to call
 * go function, it needs to be exported, see engine.gg
 * for details of this.
 * 
 * We have this one callback implemented in engine.go,
 * declaration must match the exported name.
 */
extern int rsa_sign_go_callback(
  int type, 
...
/*
 * This will be called from engine.go as a demo 
 * how it can be done. Any such function needs to
 * be declared in engine.go.
 */
void called_from_golang(){
  printf("called_from_golang: yay\n");
}


/**********/
static const char *demo_engine_id = "cms-demo-cgo";
static const char *demo_engine_name = "Demo CMS Signer Engine (C+golang)";
...
static int demo_rsa_sign(int type, const unsigned char *m, unsigned int m_length, unsigned char *sigret, unsigned int *siglen, const RSA *rsa) {
  ...
  /* call to golang - note explicit const casts */
  int ret;
  printf("demo_rsa_sign: call to golang function here.\n");
  ret = rsa_sign_go_callback(type, (unsigned char*) m, m_length, sigret, siglen, (RSA*)rsa);
